...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  /* ── Archive styles ── */
  .arc-filters{display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin-bottom:16px}
  .arc-select{padding:6px 10px;background:var(--surface2);color:var(--text);border:1px solid var(--border);border-radius:6px;font-family:inherit;font-size:11px;font-weight:500;outline:none;cursor:pointer;transition:border-color .15s}
  .arc-select:focus{border-color:var(--accent)}
  .arc-kpi-row{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:16px}
  .arc-kpi{background:var(--surface);border:1px solid var(--border);border-radius:var(--r);padding:14px 16px;border-top:3px solid var(--accent)}
  .arc-kpi-label{font-size:10px;color:var(--muted);font-weight:600;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px}
  .arc-kpi-val{font-size:26px;font-weight:700;font-family:'DM Mono',monospace}
  .arc-tbl-wrap{overflow-x:auto;border-radius:var(--r);border:1px solid var(--border)}
  .arc-tbl{width:100%;border-collapse:collapse;min-width:700px}
  .arc-tbl th{padding:9px 12px;background:var(--surface2);color:var(--muted);font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.05em;white-space:nowrap;border-bottom:1px solid var(--border)}
  .arc-tbl td{padding:8px 12px;font-size:12px;border-bottom:1px solid rgba(255,255,255,.03)}
  .arc-tbl tr:last-child td{border-bottom:none}
  .arc-empty{text-align:center;padding:60px 20px;color:var(--muted);font-size:13px}
  .pct-good{color:#10B981;font-weight:700}.pct-mid{color:#F59E0B;font-weight:700}.pct-bad{color:#EF4444;font-weight:700}
  .view-toggle{display:flex;gap:4px;background:var(--surface2);padding:3px;border-radius:7px}
  .view-btn{padding:5px 12px;border-radius:5px;border:none;background:transparent;color:var(--muted);font-size:11px;font-family:inherit;cursor:pointer;font-weight:500;transition:all .15s}
...
<div class="layout">
  <aside class="sidebar">
    ...
    <nav class="nav">
      ...
      <div class="nav-item" data-sec="archive"><span class="icon">📦</span> Lưu Trữ Kênh</div>
    </nav>
  ...
  <div class="main">
    ...
    <div class="content">
      ...
      <!-- ── SECTION: LƯU TRỮ KÊNH ── -->
      <div class="section" id="sec-archive">
        <div class="arc-filters">
          <div>
            <span style="font-size:10px;font-weight:600;color:var(--muted);text-transform:uppercase;letter-spacing:.06em;margin-right:6px">Tháng</span>
            <select id="arcMonthSel" class="arc-select" onchange="renderArchive()">
              <option value="all">Tất cả</option>
            </select>
          </div>
          <div>
            <span style="font-size:10px;font-weight:600;color:var(--muted);text-transform:uppercase;letter-spacing:.06em;margin-right:6px">Kênh</span>
            <select id="arcChannelSel" class="arc-select" onchange="renderArchive()">
              <option value="all">Tất cả</option>
            </select>
          </div>
          <div class="view-toggle">
            <button class="view-btn active" id="btnTable" onclick="setArcView('table')">Bảng</button>
            <button class="view-btn" id="btnChart" onclick="setArcView('chart')">Biểu đồ</button>
          </div>
        </div>

        <div class="arc-kpi-row" id="arcKpiRow"></div>

        <!-- Bảng chi tiết -->
        <div id="arcTableWrap" class="arc-tbl-wrap"></div>

        <!-- Biểu đồ -->
        <div id="arcChartWrap" style="display:none">
          <div class="grid-2">
            <div class="card">
              <div class="card-head"><span class="card-title">Video Hoàn Thành vs Mục Tiêu</span></div>
              <div class="chart-box h240"><canvas id="arcChartVideo"></canvas></div>
            </div>
            <div class="card">
              <div class="card-head"><span class="card-title">Kịch Bản Hoàn Thành vs Mục Tiêu</span></div>
              <div class="chart-box h240"><canvas id="arcChartKb"></canvas></div>
            </div>
          </div>
          <div class="card" style="margin-top:14px">
            <div class="card-head"><span class="card-title">Tổng View theo Kênh</span></div>
            <div class="chart-box h240"><canvas id="arcChartViews"></canvas></div>
          </div>
        </div>
      </div>
...
<script>
let ALL = null, curWeek = 0, curSec = 'overview';
const charts = {};
const WEEK_COLORS = ['#3B82F6','#10B981','#F59E0B','#EF4444','#8B5CF6'];
const BR_COLORS   = ['#6C63FF','#10B981','#F59E0B','#EF4444','#8B5CF6','#06B6D4'];
const SECTION_TITLES = { overview:'Tổng Quan', casestudy:'Casestudy (Form 4F)', videoview:'Video & View', kichban:'Kịch Bản', broll:'B-Roll', archive:'Lưu Trữ Kênh' };

// ── ARCHIVE STATE ──
let _arcData = [], _arcView = 'table', _arcLoaded = false;
...
function loadData() {
  ...
  google.script.run.withSuccessHandler(function(res) {
    ...
    document.getElementById('metaMonth').textContent = ALL.meta.month;
    document.getElementById('updatedAt').textContent = 'Cập nhật: ' + ALL.meta.updatedAt;
    renderSection();
...
function refreshCurrent() {
  if (curSec === 'archive') { _arcLoaded = false; loadArchive(); return; }
  changeMonth(document.getElementById('monthSel').value);
}
...
document.querySelectorAll('.nav-item').forEach(el => {
  el.addEventListener('click', () => {
    ...
    if (curSec === 'archive') { loadArchive(); return; }
...
function renderSection() {
  ...
  else if (curSec === 'casestudy') renderCasestudy();
  else if (curSec === 'videoview') renderVideoView();
  else if (curSec === 'kichban')   renderKichBan();
  else if (curSec === 'broll')     renderBRoll();
}
...
function renderOverview() {
  ...
  document.getElementById('overviewKpi').innerHTML = ALL.casestudy.map((w,i) =>
    `<div class="kpi"><span class="kpi-w" style="color:${WEEK_COLORS[i%4]}">${w.label}</span><span class="kpi-n">${w.total}</span><span class="kpi-s">${ALL.videoview[i].totalDone||0} video · ${fmtNum(ALL.videoview[i].totalViews||0)} views</span></div>`
  ).join('');

  destroyChart('overview');
  charts['overview'] = new Chart(document.getElementById('chartOverview'), { type:'bar', data:{ labels, datasets:[
    {label:'Hoàn thành',data:vidDone,backgroundColor:WEEK_COLORS.map(c=>c+'99'),borderColor:WEEK_COLORS,borderWidth:1.5,borderRadius:5},
    {label:'Mục tiêu',data:vidTgt,backgroundColor:'rgba(139,147,168,.1)',borderColor:'#8B93A8',borderWidth:1.5,borderRadius:5,borderDash:[4,4]}
  ]}, options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:true,labels:{color:'#8B93A8'}}},scales:{x:{grid:{display:false}},y:{grid:{color:'rgba(255,255,255,.04)'}}}}});
  ...
  document.getElementById('overviewKB').innerHTML = ALL.kichban.map((w,i) =>
    `<div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid rgba(255,255,255,.04)"><span style="font-size:11px;color:${WEEK_COLORS[i%4]};font-weight:600;min-width:48px">${w.label}</span><div style="flex:1">${progBar(w.totalDone,w.totalTarget,WEEK_COLORS[i%4])}</div><span style="font-size:11px;color:#8B93A8;min-width:60px;text-align:right">${w.totalDone}/${w.totalTarget} KB</span></div>`
  ).join('');
}
...
function renderCasestudy() {
  ...
  document.querySelector('#csTable tbody').innerHTML = d.members.map(m =>
    `<tr><td><span class="avatar">${initials(m.name)}</span>${m.name}</td><td style="font-family:'DM Mono',monospace;font-weight:500;color:${wc}">${m.count}</td><td>${progBar(m.count,d.total,wc)}</td><td style="color:#8B93A8;font-size:11px">${m.note||'—'}</td></tr>`
  ).join('') + `<tr class="total-row"><td colspan="1">TỔNG</td><td>${d.total}</td><td colspan="2"></td></tr>`;

  destroyChart('cs');
  if (d.members.length) charts['cs'] = new Chart(document.getElementById('chartCS'), {type:'doughnut',data:{labels:d.members.map(m=>m.name.split(' ').slice(-1)[0]),datasets:[{data:d.members.map(m=>m.count),backgroundColor:WEEK_COLORS,borderWidth:2,borderColor:'#161B27'}]},options:{responsive:true,maintainAspectRatio:false,cutout:'68%',plugins:{legend:{position:'right',labels:{color:'#8B93A8',font:{size:10},boxWidth:10}}}}});
}
...
function renderVideoView() {
  ...
  document.querySelector('#vvTable tbody').innerHTML = d.channels.map((c,i) =>
  ...
  ).join('') + `<tr class="total-row"><td>TỔNG</td><td style="font-family:'DM Mono',monospace;font-size:13px"><span style="color:#E8EAF0;font-weight:600">${d.totalDone}</span> <span style="color:#8B93A8;font-size:11px">/ ${d.totalTarget}</span></td><td>${progBar(d.totalDone,d.totalTarget,wc)}</td><td>${d.totalViews.toLocaleString('vi-VN')}</td><td>—</td></tr>`;

  destroyChart('vv');
  charts['vv'] = new Chart(document.getElementById('chartVV'), {type:'bar',data:{labels:d.channels.map(c=>c.name),datasets:[{label:'View',data:d.channels.map(c=>c.views),backgroundColor:WEEK_COLORS.slice(0,d.channels.length).map(c=>c+'88'),borderColor:WEEK_COLORS,borderWidth:1.5,borderRadius:6}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{grid:{display:false}},y:{grid:{color:'rgba(255,255,255,.04)'},ticks:{callback:v=>fmtNum(v)}}}}});
}
...
function renderKichBan() {
  ...
  document.querySelector('#kbTable tbody').innerHTML = d.channels.map((c,i) => {
    ...
    if (c.items.length===0) rowsHTML += `<tr><td rowspan="2" style="vertical-align:top;padding-top:12px;border-right:1px solid rgba(255,255,255,0.05)"><span class="dot" style="background:${WEEK_COLORS[i%4]}"></span><b>${c.name}</b></td><td colspan="3" style="color:#8B93A8;font-style:italic">Chưa nhập dữ liệu</td></tr>`;
    rowsHTML += `<tr class="total-row" style="background:rgba(255,255,255,0.03)"><td><b style="color:${WEEK_COLORS[i%4]}">TỔNG (${c.name})</b></td><td style="font-family:'DM Mono',monospace;font-size:13px"><span style="color:${WEEK_COLORS[i%4]};font-weight:600">${c.done}</span> <span style="color:#8B93A8;font-size:11px">/ ${c.target}</span></td><td>${progBar(c.done,c.target,WEEK_COLORS[i%4])}</td></tr>`;
    return rowsHTML;
  }).join('');

  destroyChart('kb');
  if (d.channels.length) charts['kb'] = new Chart(document.getElementById('chartKB'), {type:'radar',data:{labels:d.channels.map(c=>c.name),datasets:[{label:'Hoàn thành',data:d.channels.map(c=>c.done),borderColor:wc,backgroundColor:wc+'22',pointBackgroundColor:wc,borderWidth:2,pointRadius:4},{label:'Mục tiêu',data:d.channels.map(c=>c.target),borderColor:'#8B93A8',backgroundColor:'transparent',borderDash:[4,4],borderWidth:1.5}]},options:{responsive:true,maintainAspectRatio:false,scales:{r:{angleLines:{color:'rgba(255,255,255,.06)'},grid:{color:'rgba(255,255,255,.06)'},ticks:{display:false}}},plugins:{legend:{labels:{color:'#8B93A8',boxWidth:10}}}}});
}
...
function renderBRoll() {
  ...
  document.querySelector('#brTable tbody').innerHTML = d.items.map((it,i) =>
    `<tr><td><span class="dot" style="background:${BR_COLORS[i%6]}"></span>${it.type}</td><td style="font-family:'DM Mono',monospace;color:${BR_COLORS[i%6]}">${it.qty}</td><td>${progBar(it.qty,d.total,BR_COLORS[i%6])}</td><td style="color:#8B93A8;font-size:11px">${it.note||'—'}</td></tr>`
  ).join('') + `<tr class="total-row"><td>TỔNG</td><td>${d.total}</td><td colspan="2"></td></tr>`;

  destroyChart('br');
  if (d.items.length) {
    charts['br'] = new Chart(document.getElementById('chartBR'), {type:'doughnut',data:{labels:d.items.map(i=>i.type),datasets:[{data:d.items.map(i=>i.qty),backgroundColor:BR_COLORS,borderWidth:2,borderColor:'#161B27'}]},options:{responsive:true,maintainAspectRatio:false,cutout:'60%',plugins:{legend:{display:false}}}});
    document.getElementById('brLegend').innerHTML = d.items.map((it,i) =>
      `<div class="donut-item"><span><span class="dot" style="background:${BR_COLORS[i%6]}"></span>${it.type}</span><span style="font-family:'DM Mono',monospace;font-size:11px;color:#8B93A8">${it.qty}</span></div>`
    ).join('');
  }
}
...
function loadArchive() {
  document.querySelectorAll('.section').forEach(s => s.classList.remove('active'));
  document.getElementById('sec-archive').classList.add('active');
  if (_arcLoaded) { renderArchive(); return; }
  document.getElementById('arcTableWrap').innerHTML = '<div class="loading" style="height:200px"><div class="spinner"></div><span>Đang tải dữ liệu lưu trữ...</span></div>';
  google.script.run
    ...
    .withFailureHandler(function(e) {
      document.getElementById('arcTableWrap').innerHTML = '<div class="error-box">⚠️ Lỗi tải dữ liệu: ' + (e.message||e) + '</div>';
    })
    .getChannelArchiveData();
}

function _populateArcFilters() {
  const months   = [...new Set(_arcData.map(r => r.month))].sort();
  const channels = [...new Set(_arcData.map(r => r.channel))];
  const mSel = document.getElementById('arcMonthSel');
  const cSel = document.getElementById('arcChannelSel');
  mSel.innerHTML = '<option value="all">Tất cả</option>' + months.map(m => `<option value="${m}">${m}</option>`).join('');
  cSel.innerHTML = '<option value="all">Tất cả</option>' + channels.map(c => `<option value="${c}">${c}</option>`).join('');
}

function _arcFiltered() {
  const m = document.getElementById('arcMonthSel').value;
  const c = document.getElementById('arcChannelSel').value;
  return _arcData.filter(r => (m==='all'||r.month===m) && (c==='all'||r.channel===c));
}
...
function renderArchive() {
  const rows = _arcFiltered();
  _renderArcKpi(rows);
  document.getElementById('arcTableWrap').style.display = _arcView==='table' ? '' : 'none';
  document.getElementById('arcChartWrap').style.display  = _arcView==='chart' ? '' : 'none';
  if (_arcView==='table') _renderArcTable(rows);
  else                    _renderArcCharts(rows);
}

function _renderArcKpi(rows) {
  const totalVideo = rows.reduce((s,r)=>s+r.videoDone,0);
  const totalViews = rows.reduce((s,r)=>s+r.views,0);
  const totalKb    = rows.reduce((s,r)=>s+r.kbDone,0);
  const avgPctV    = rows.length ? rows.reduce((s,r)=>s+r.videoPct,0)/rows.length : 0;
  const kpis = [
    {label:'Tổng Video HT',  val: totalVideo,                            color:'#3B82F6', fmt:'n'},
    {label:'Tổng Views',     val: totalViews.toLocaleString('vi-VN'),    color:'#10B981', fmt:'raw'},
    {label:'Tổng KB HT',     val: totalKb,                               color:'#F59E0B', fmt:'n'},
    {label:'TB % Video',     val: Math.round(avgPctV*100)+'%',           color:'#6C63FF', fmt:'raw'},
  ];
  document.getElementById('arcKpiRow').innerHTML = kpis.map(k =>
    `<div class="arc-kpi" style="border-top-color:${k.color}">
       <div class="arc-kpi-label">${k.label}</div>
       <div class="arc-kpi-val" style="color:${k.color}">${k.fmt==='raw'?k.val:k.val}</div>
     </div>`
  ).join('');
}

function _pctClass(v) { return v>=0.8?'pct-good':v>=0.5?'pct-mid':'pct-bad'; }

function _renderArcTable(rows) {
  if (!rows.length) {
    document.getElementById('arcTableWrap').innerHTML = '<div class="arc-empty">Chưa có dữ liệu lưu trữ.<br>Hãy chốt sổ tháng qua menu <b>Báo Cáo → Lưu trữ & Chốt sổ</b>.</div>';
    return;
  }
  const ths = ['Tháng','Kênh','Tuần','Video HT','Video MT','% Video','Tổng View','KB HT','KB MT','% KB']
    .map(h=>`<th>${h}</th>`).join('');
  const trs = rows.map(r => {
    const pV = Math.round(r.videoPct*100), pK = Math.round(r.kbPct*100);
    return `<tr>
      <td style="color:var(--muted);font-size:11px">${r.month}</td>
      <td style="font-weight:600"><span class="dot" style="background:${WEEK_COLORS[_arcData.map(x=>x.channel).indexOf(r.channel)%4]}"></span>${r.channel}</td>
      <td style="color:var(--muted);font-size:11px;text-align:center">${r.week}</td>
      <td style="text-align:center;font-family:'DM Mono',monospace">${r.videoDone}</td>
      <td style="text-align:center;font-family:'DM Mono',monospace;color:var(--muted)">${r.videoTarget}</td>
      <td style="text-align:center" class="${_pctClass(r.videoPct)}">${pV}%</td>
      <td style="text-align:right;font-family:'DM Mono',monospace">${r.views.toLocaleString('vi-VN')}</td>
      <td style="text-align:center;font-family:'DM Mono',monospace">${r.kbDone}</td>
      <td style="text-align:center;font-family:'DM Mono',monospace;color:var(--muted)">${r.kbTarget}</td>
      <td style="text-align:center" class="${_pctClass(r.kbPct)}">${pK}%</td>
    </tr>`;
  }).join('');
  document.getElementById('arcTableWrap').innerHTML =
    `<table class="arc-tbl"><thead><tr>${ths}</tr></thead><tbody>${trs}</tbody></table>`;
}

function _renderArcCharts(rows) {
  // Gộp theo kênh
  const chMap = {};
  rows.forEach(r => {
    if (!chMap[r.channel]) chMap[r.channel] = {videoDone:0,videoTarget:0,views:0,kbDone:0,kbTarget:0};
    chMap[r.channel].videoDone   += r.videoDone;
    chMap[r.channel].videoTarget += r.videoTarget;
    chMap[r.channel].views       += r.views;
    chMap[r.channel].kbDone      += r.kbDone;
    chMap[r.channel].kbTarget    += r.kbTarget;
  });
  const labels = Object.keys(chMap);
  const CHC = labels.map((_,i)=>WEEK_COLORS[i%WEEK_COLORS.length]);

  const chartOpts = (title) => ({
    responsive:true, maintainAspectRatio:false,
    plugins:{legend:{labels:{color:'#8B93A8',boxWidth:10}},title:{display:false}},
    scales:{x:{grid:{display:false},ticks:{color:'#8B93A8'}},y:{grid:{color:'rgba(255,255,255,.04)'},ticks:{color:'#8B93A8'}}}
  });

  destroyChart('arcV');
  charts['arcV'] = new Chart(document.getElementById('arcChartVideo'), {
    type:'bar',
    data:{ labels, datasets:[
      {label:'Video Hoàn Thành',data:labels.map(l=>chMap[l].videoDone),  backgroundColor:CHC.map(c=>c+'BB'),borderColor:CHC,borderWidth:1.5,borderRadius:5},
      {label:'Video Mục Tiêu',  data:labels.map(l=>chMap[l].videoTarget),backgroundColor:'rgba(139,147,168,.12)',borderColor:'#8B93A8',borderWidth:1.5,borderRadius:5}
    ]},
    options: chartOpts()
  });

  destroyChart('arcK');
  charts['arcK'] = new Chart(document.getElementById('arcChartKb'), {
    type:'bar',
    data:{ labels, datasets:[
      {label:'KB Hoàn Thành',data:labels.map(l=>chMap[l].kbDone),   backgroundColor:CHC.map(c=>c+'BB'),borderColor:CHC,borderWidth:1.5,borderRadius:5},
      {label:'KB Mục Tiêu',  data:labels.map(l=>chMap[l].kbTarget), backgroundColor:'rgba(139,147,168,.12)',borderColor:'#8B93A8',borderWidth:1.5,borderRadius:5}
    ]},
    options: chartOpts()
  });

  destroyChart('arcViews');
  charts['arcViews'] = new Chart(document.getElementById('arcChartViews'), {
    type:'bar',
    data:{ labels, datasets:[{
      label:'Tổng View',
      data:labels.map(l=>chMap[l].views),
      backgroundColor:CHC.map(c=>c+'99'),
      borderColor:CHC, borderWidth:1.5, borderRadius:6
    }]},
    options:{...chartOpts(), plugins:{legend:{display:false}}, scales:{x:{grid:{display:false},ticks:{color:'#8B93A8'}},y:{grid:{color:'rgba(255,255,255,.04)'},ticks:{color:'#8B93A8',callback:v=>fmtNum(v)}}}}
  });
}
